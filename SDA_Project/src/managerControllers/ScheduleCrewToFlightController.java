...
public class ScheduleCrewToFlightController {
	@FXML
...
//        menuItemAddEmployee.setOnAction(event -> addEmployee());
		...
		try {
			...
			List<Flight> filteredFlights = flights.stream()
					.filter(flight -> !flight.getStatus().equals("Cancelled") && !flight.getStatus().equals("Completed"))
					.collect(Collectors.toList());


			ObservableList<Flight> observableFlights = FXCollections.observableArrayList(filteredFlights);
		...
		selectflightbutton.setOnAction(event -> NextButtonClick());
	}
	...
	private void NextButtonClick() {
		...
		if (selectedFlight != null) {
			String flightID = selectedFlight.getFlightNumber(); // Assuming getFlightID() returns the ID of the flight

			try {
				// Load the ScheduleNewFlight.fxml file
				FXMLLoader loader = new FXMLLoader(getClass().getResource("/managerViews/AssignCrewToFlight.fxml"));
				Parent root = loader.load();

				AssignCrewToFlightController assignCrewToFlightController = loader.getController();
				assignCrewToFlightController.setFlightId(flightID);
				...
				Stage currentStage = (Stage) selectflightbutton.getScene().getWindow();
				currentStage.hide();
			...
			// Show an alert or message indicating that no flight is selected
			Alert alert = new Alert(AlertType.WARNING);
			alert.setTitle("No Flight Selected");
			alert.setHeaderText(null);
			alert.setContentText("Please select a flight before proceeding.");
			alert.showAndWait();
		}
	}
